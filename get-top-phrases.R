...
# ukládání do PDFka
pdf( paste("./exports/top-queries-", str_replace_all(siteUrl, "[.:/]" ,"-"),"-", dateTo, ".pdf", sep=""), onefile = TRUE, width=16 , height=10)
...
for (query in data$query) {
  ...
  query_data <- get_data_per_keyword(query)

  # sort dat podle datumu
  query_data <- query_data[order(query_data$date),]
  
  # příprava dat pro grafování
  gsc.data <- data.frame(date = query_data$date, 
  ...
  p1 <- ggplot( gsc.data, aes(x=date)) + 
    ...
    labs( title = paste0 (query, "; kliků: ", sum(gsc.data$clicks), 
